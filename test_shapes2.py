...
if __name__ == "__main__":
    ...
    diminsions = SimpleNamespace(r=27, l=29, N_edge=10, Lx = 27, L=20, W=8)
    diminsions.N_edge = 40
    
    # Create the system
    sys1 = make_system(a, shape, diminsions)
    sys2 = make_system(a, shape2, diminsions)
    

    # Plot the system
    # =======================================================================================================================
    kwant.plot(sys1, show=True)
    kwant.plot(sys2, show=True)
    ...
    # =======================================================================================================================
    final_sys = sys1.finalized()
    final_sys2 = sys2.finalized()
    if len(final_sys.sites) <= 2**12:
        h = final_sys.hamiltonian_submatrix()
        h2 = final_sys2.hamiltonian_submatrix()
        eigenenergies = np.linalg.eigvalsh(h)
        eigenenergies2 = np.linalg.eigvalsh(h2)
    else:
        print(f"Using sparse matrix sizes: {len(final_sys.sites)} and {len(final_sys2.sites)}")
        h = final_sys.hamiltonian_submatrix(sparse=True)
        h2 = final_sys2.hamiltonian_submatrix(sparse=True)
        eigenenergies = sla.eigs(h, k=h.shape[0] - 2, return_eigenvectors=False)
        sort_arg = np.argsort(eigenenergies)
        eigenenergies = eigenenergies[sort_arg]
        eigenenergies2 = sla.eigs(h2, k=h2.shape[0] - 2, return_eigenvectors=False)
        sort_arg2 = np.argsort(eigenenergies2)
        eigenenergies2 = eigenenergies2[sort_arg2]
    ...
    plt.figure()
    plt.hist(eigenenergies, bins=100, density=True, histtype='step', cumulative=False, label="zigzag")
    plt.hist(eigenenergies2, bins=100, density=True, histtype='step', cumulative=False, label="armchair")
    plt.xlabel('Energy')
