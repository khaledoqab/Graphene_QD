...
if __name__ == "__main__":
    ...
    diminsions = SimpleNamespace(r=15, l=29, N_edge=10, Lx = 20, L=20, W=8)
    # diminsions.N_edge = 6
    ...
    # Symmetrize the eigenstates
    # =======================================================================================================================
    # zero_energy_indices = np.where(np.abs(eigenenergies) < 1e-5)[0]
    # R = np.zeros((len(eigenstates), len(eigenstates)), dtype=complex)
    # x0, y0 = b_lat(3, 3).pos
    # for i in range(len(eigenstates)):
    #     site_to_transform = final_sys.sites[i]
    #     pos_to_transform = site_to_transform.pos
    #     x, y = pos_to_transform
    #     x = x - x0
    #     y = y - y0
    #     r120 = np.array([[np.cos(2*np.pi/3), -np.sin(2*np.pi/3)], [np.sin(2*np.pi/3), np.cos(2*np.pi/3)]])
    #     x, y = r120@np.array([x, y])
    #     x = x + x0
    #     y = y + y0
    #     # find the corresponding site
    #     for site in final_sys.sites:
    #         if abs(site.pos[0]-x)<1e-2 and abs(site.pos[1]-y) <1e-2:
    #             target_site = site
    #             break
    #     index_of_target_site = final_sys.sites.index(target_site)
    #     R[i, index_of_target_site] = 1

        

    # R_ = eigenstates[np.array(zero_energy_indices)]@ R @ np.conjugate(eigenstates[np.array(zero_energy_indices)]).T
    # symmetrizing_eigenvalues, symmetrized_eigenstates = np.linalg.eig(R_)

    # symmetric_eigenstates = []
    # for comb in symmetrized_eigenstates.T:
    #     symmetric_eigenstate = np.zeros(len(eigenstates), dtype=complex)
    #     for i, k in enumerate(comb):
    #         symmetric_eigenstate += k * eigenstates[zero_energy_indices[i]]
    #     symmetric_eigenstates.append(symmetric_eigenstate)
    # symmetric_eigenstates = np.array(symmetric_eigenstates)
    
    # print("Symmetrized eigenvalues: ", symmetrizing_eigenvalues)
    # M = np.zeros((len(eigenstates), len(eigenstates)), dtype=complex)
    # x0, y0 = b_lat(3, 3).pos
    # for i in range(len(eigenstates)):
    #     site_to_transform = final_sys.sites[i]
    #     pos_to_transform = site_to_transform.pos
    ...
    #             break
    #     index_of_target_site = final_sys.sites.index(target_site)
    #     M[i, index_of_target_site] = 1

    # print(np.allclose(R@M + M@R, 0))

    # M_ = eigenstates[zero_energy_indices]@ M @ np.conjugate(eigenstates[zero_energy_indices]).T
    # M_1 = symmetric_eigenstates[[0, 2]]@ M @ np.conjugate(symmetric_eigenstates[[0, 2]]).T
    # M_2 = symmetric_eigenstates[[1, 3]]@ M @ np.conjugate(symmetric_eigenstates[[1, 3]]).T
    # M_3 = symmetric_eigenstates[[4, 5, 6]]@ M @ np.conjugate(symmetric_eigenstates[[4, 5, 6]]).T
    ...
    # s_ = np.sin(angle2)
    # c_ = np.cos(angle2)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[0]/np.sqrt(2) + np.exp(0.20j*np.pi)*symmetric_eigenstates[2]/np.sqrt(2))**2, colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[0]/np.sqrt(2) + np.exp(-3j*np.pi/10)*symmetric_eigenstates[2]/np.sqrt(2))**2, colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, np.abs(c*symmetric_eigenstates[0] + np.exp(1j*phi)*s*symmetric_eigenstates[2])**2, colorbar=True, show=True, oversampling=10) # angle = pi/5, phi = 1.07*pi/9
    # kwant.plotter.map(final_sys, np.abs(c*symmetric_eigenstates[1] + np.exp(1j*phi)*s*symmetric_eigenstates[3])**2, colorbar=True, show=True, oversampling=10) # angle = 0.16*pi, phi = 0.0
    # kwant.plotter.map(final_sys, np.abs(c*symmetric_eigenstates[1] + np.exp(1j*phi)*s*symmetric_eigenstates[3])**2, colorbar=True, show=True, oversampling=10) # angle = 0.25*pi, phi = 0.768*pi
    # kwant.plotter.map(final_sys, np.abs(c*c_*symmetric_eigenstates[4] + np.exp(1j*phi)*c_*s*symmetric_eigenstates[5] + np.exp(1j*phi2)*s_*symmetric_eigenstates[6])**2, colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[1])**2, colorbar=True, show=True)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[2])**2, colorbar=True, show=True)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[3])**2, colorbar=True, show=True)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[4])**2, colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[5])**2, colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, np.abs(symmetric_eigenstates[6])**2, colorbar=True, show=True, oversampling=10)
    # amp_sq = np.abs(symmetric_eigenstates[4] - 0.74*symmetric_eigenstates[6])**2
    # kwant.plotter.map(final_sys, amp_sq/np.linalg.norm(amp_sq), colorbar=True, show=True, oversampling=10)

    # kwant.plotter.map(final_sys, (symmetric_eigenstates[4]), colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, (symmetric_eigenstates[5]), colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, (symmetric_eigenstates[6]), colorbar=True, show=True, oversampling=10)
