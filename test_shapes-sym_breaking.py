...
def make_system(a, shape="ribbon", diminsions=SimpleNamespace(L=10, W=10), phi=0.0, t_2=0.0, u_s=0.0):
    ...
    def onsite(site):
        return u_s if site.family == a_lat else -u_s
    ...
    def nnn_hopping(site1, site2):
        x1, y1 = site1.pos
        x2, y2 = site2.pos
        return -t_2*np.exp(-1j*phi/2*(x2 - x1)*(y1 + y2))
...
if __name__ == "__main__":
    ...
    diminsions = SimpleNamespace(r=15, l=10, N_edge=10, Lx = 15, L=20, W=8)
    # diminsions.N_edge = 30
    
    # Create the system
    sys = make_system(a, shape, diminsions, phi=0.2, t_2=1e-12+0., u_s=0.0)


    # comparing modes energies
    # ========================================================================================================================

    # sys2 = make_system(a, shape, diminsions, phi=0.3, t_2=1e-12, u_s=0.0)
    # sys3 = make_system(a, shape, diminsions, phi=0.5, t_2=1e-12, u_s=0.0)
    # sys4 = make_system(a, shape, diminsions, phi=0.9, t_2=1e-12, u_s=0.0)

    plt.figure()
    plt.xlabel('index')
    plt.ylabel('Energy')
    plt.grid()
    plt.plot(get_eigenvalues(sys), 'o', label='B=0.1', alpha=0.7)
    # plt.plot(get_eigenvalues(sys2), 'x', label='B=0.3', alpha=0.7, color='red')
    # plt.plot(get_eigenvalues(sys3), '.', label='B=0.5', alpha=0.7, color='green')
    # plt.plot(get_eigenvalues(sys4), 's', label='B=0.9', alpha=0.3, color='orange')
    plt.legend()
    # # plt.savefig(f"./figures/eigenenergies_{shape}-s.pdf")
    plt.show()
    ...
    # sym_state = np.abs(eigenstates[48] + 1j*eigenstates[49])
    # sym_state = np.abs(eigenstates[48] - 1j*eigenstates[49])
    ...
    # ========================================================================================================================
    # Triangular-zz plots of the zero energy eigenstate
    # kwant.plotter.map(final_sys, np.abs(eigenstates[45])**2, colorbar=True, show=True, oversampling=10)
    # sym_state = np.abs(eigenstates[46] + 1j*eigenstates[47])**2
    # kwant.plotter.map(final_sys, sym_state/np.linalg.norm(sym_state), colorbar=True, show=True, oversampling=10)
    # sym_state = np.abs(eigenstates[46] - 1j*eigenstates[47])**2
    # kwant.plotter.map(final_sys, sym_state/np.linalg.norm(sym_state), colorbar=True, show=True, oversampling=10)
    # sym_state = np.abs(eigenstates[48] + 1j*eigenstates[49])**2
    # kwant.plotter.map(final_sys, sym_state/np.linalg.norm(sym_state), colorbar=True, show=True, oversampling=10)
    # sym_state = np.abs(eigenstates[48] - 1j*eigenstates[49])**2
    # kwant.plotter.map(final_sys, sym_state/np.linalg.norm(sym_state), colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, np.abs(eigenstates[50])**2, colorbar=True, show=True, oversampling=10)
    # kwant.plotter.map(final_sys, np.abs(eigenstates[51])**2, colorbar=True, show=True, oversampling=10)
    # =======================================================================================================================
