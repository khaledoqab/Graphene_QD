...
def onsite(site, param):
    staggered_pot =  param.u_s if site.family.name == 'a' else -param.u_s
...
def nnn_hopping(site1, site2, param):
    x1, y1 = site1.pos
    x2, y2 = site2.pos
    return param.t_2#*np.exp(-1j*param.phi/2*(x2 - x1)*(y1 + y2))
...
def make_system(a, shape="ribbon", diminsions=SimpleNamespace(L=10, W=10)):#, phi=0.0, t_2=0.0, u_s=0.0):
    ...
    sys[lat.neighbors(1)] = nn_hopping
...
def edge_prob(syst: kwant.Builder, eig_state: np.ndarray) -> float:
    ...
    for i, s in enumerate(sites):
        ...
        if num_connections <9:
            edge_prob= edge_prob + (np.abs(eig_state[i])**2)
...
if __name__ == "__main__":
    ...
    parameters = SimpleNamespace(phi=0.0, t_2=1e-12+0.2j, u_s=0.0, disorder=0.0)
    shape = shapes[0]
    ...
    index = 46#binary_search(eigs_soc, 0.0)
    image = np.abs(states_soc[index])**2# + np.abs(states_soc[index])**2
    image_nosoc = np.abs(states_nosoc[index])**2# + np.abs(states_nosoc[index])**2
    ...
    index = 45
    image = np.abs(states_soc[index])**2
    ...
    # PRs = []
    # soc_coeffs = np.linspace(0.001, 1.0, 100)
    # for t_so in soc_coeffs:
    #     PRs_ = []
    #     for s in range(45, 52, 1):
    ...
    while False:
        ...
        if ind.strip() == "-1":
            ...
            kwant.plotter.map(final_sys, np.abs(state)**2 + np.abs(state)**2, colorbar=True, show=False, oversampling=10, ax=ax1)
            ax1.set_title("SOC")
            kwant.plotter.map(final_sys, np.abs(states_nosoc[ind])**2 + np.abs(states_nosoc[ind])**2, colorbar=True, show=False, oversampling=10, ax=ax2)
